...
class RBFModulePyTorch(nn.Module):
    def __init__(self, input_features, output_features, lengthscale_init='auto', tunable_kernel=False, dtype=torch.FloatTensor):
        ...
        # DGP Initialize lengthscales to sqrt(D) / 2. sklearn initializes to sqrt(D/2).
        # If tunable is set to True, they can be learned.
        # lengthscale_init = (0.5 * torch.log(T([input_features]).type(torch.FloatTensor)) - torch.log(T([2.0]))).type(torch.FloatTensor)
        # sklearn lengthscale works slightly better it seems!
        if lengthscale_init == 'auto':
            lengthscale_init = 0.5 * torch.log(T([input_features]).type(dtype) / 2.)
        
        self.log_lengthscales = nn.Parameter(
            torch.ones(input_features).type(dtype) * lengthscale_init,
        ...
        def forward(self, data):
            ...
            input = input / torch.exp(self.log_lengthscales)

            output = torch.cos(self.proj(input) + self.bias)
...
def project_big_np_matrix(data, out_dim=int(1e4), chunk_size=int(1e4), projection='opu',
    ...
    for i in range(n_chunks):
        ...
        if framework=='pytorch':
            data_chunk = torch.from_numpy(data_chunk)
            if cuda:
